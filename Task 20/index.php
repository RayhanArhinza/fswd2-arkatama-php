...
// Cek apakah waktu terakhir akses tersimpan dalam session
if (isset($_SESSION['last_access_time'])) {
    $lastAccessTime = $_SESSION['last_access_time'];
    $currentTime = time();

    // Tentukan waktu idle (dalam detik)
    $idleTime = 600; // Misalnya 1 jam

    // Cek apakah waktu idle sudah mencapai batas yang ditentukan
    if ($currentTime - $lastAccessTime > $idleTime) {
        // Jika sudah mencapai batas, lakukan logout
        logout();
    }
}

// Simpan waktu terakhir akses ke session
$_SESSION['last_access_time'] = time();

// Kode di bawah ini hanya dieksekusi jika pengguna sudah login
...
<section>
    ...
    // Fungsi untuk menampilkan pesan session berakhir
    function displayLogoutMessage() {
        ...
        function setTimeoutLogout() {
            var timeoutInMinutes = 1; // Waktu idle (dalam menit)
            var timeoutInMillis = timeoutInMinutes * 60 * 1000; // Konversi ke milidetik

            var timeoutID;

            // Fungsi untuk mengatur timer
            function startTimer() {
                timeoutID = setTimeout(displayLogoutMessage, timeoutInMillis);
            }
            ...
            // Event listener untuk mereset timer saat ada interaksi pengguna
        ...
        // Panggil fungsi setTimeoutLogout saat halaman selesai dimuat
        window.onload = setTimeoutLogout;
